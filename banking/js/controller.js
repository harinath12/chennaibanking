...
.controller('CCFormController', function($scope, httpService, $timeout) {
	...
	$scope.setInsurace = function(insurance){
		$scope.newEnquiry.insurance = insurance;
	};
	$scope.verify_otp = function(){
		if($scope.pageInfo.otp == $scope.pageInfo.verifyotp){
			...
			httpService.post(ajaxurl+'cb_update_verified', {id: $scope.logid, mobile: $scope.newEnquiry.mobile}).then(function(res){
